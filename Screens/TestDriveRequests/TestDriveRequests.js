...
const TestDriveRequests = props => {
  const userInfo = useSelector(state => state.auth.userInfo);
  const allGetInTouchData = useSelector(state => state.data.getInTuchData);
  const gettestdrivedata = useSelector(state => state.data.gettestdrivedata);
  ...
  useEffect(() => {
    const getAllNtification = async () => {
      await dispatch(
        AllNotification.getTestdriverequests(userInfo?.access_token, page),
      );
    };
    getAllNtification();
  }, [page]);
  ...
  useEffect(() => {
    if(requestresponse == "success" ){
      ...
      dispatch(
        AllNotification.storegettestdrivedata(data)
      );
       dispatch(
...
top: -2
  ...
  const fetchData = () => {
    dispatch(AllNotification.getTestdriverequests(userInfo?.access_token, 1));
    setIsFetching(false);
  };
  const onRefresh = () => {
 
    dispatch(AllNotification.clearTestdriveRequests());
...
<StatusChangeModal
        ...
        onPress={(status) => {
        setstatus(status)
            dispatch(AllNotification.updateRequestsstatus(userInfo?.access_token, "TestDriveStatus", status, itemid));
