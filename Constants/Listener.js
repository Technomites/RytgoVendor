...
export default listeners = () => {
  ...
  global.notifListener = messaging().onMessage(async remoteMessage => {
    __DEV__ &&   console.log('on screen messages' + JSON.stringify(remoteMessage));

 
    
    PushNotification.localNotification({
      // Android Only Properties /
      largeIcon: remoteMessage?.data?.fcm_options?.image, // (optional) default: "ic_launcher". Use "" for no large icon.
  ...
  PushNotification.configure({
    ...
    onRegister: () => {
      __DEV__ &&    console.log('REGISTRED');
    },
    popInitialNotification: async notification => {
      __DEV__ &&   console.log('popInitialNotification:', notification);
    },
