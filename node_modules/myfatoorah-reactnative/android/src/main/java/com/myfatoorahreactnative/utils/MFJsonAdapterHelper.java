...
public class MFJsonAdapterHelper<T> implements JsonDeserializer<T> {
  private boolean isUpperCaseJson = true;
  ...
  public String convertJsonStringToLowercase(String jsonString) {
    isUpperCaseJson = false;
    JsonObject jsonObject = new JsonParser().parse(jsonString).getAsJsonObject();
    JsonObject map = GenerateSubClass(jsonObject);
    return new Gson().toJson(map);
  }

  public String convertJsonStringToUppercase(String jsonString) {
    isUpperCaseJson = true;
    JsonObject jsonObject = new JsonParser().parse(jsonString).getAsJsonObject();
    JsonObject map = GenerateSubClass(jsonObject);
    return new Gson().toJson(map);
  }
